          ...
          foreach ( $woocommerce_cart_contents as $cart_item_key => $cart_item ) {
              ...
              if ($cart_item['variation_id'] > ' ') {      
                  ...
                  if ( version_compare( WC_VERSION, '3.3.0', '>=' ) ) {
                    $vtwpr_cart_item->product_name  = $_product->get_title().wc_get_formatted_cart_item_data($cart_item); 
                  } else {
                    $vtwpr_cart_item->product_name  = $_product->get_title().$woocommerce->cart->get_item_data( $cart_item ); 
                  }
                  //v1.0.62 end                   
 ...
 */
  ...
  //error_log( print_r(  'vtwpr_load_vtwpr_cart_for_single_product_price, product_id= ' .$product_id .' price= ' .$price, true ) ); 
      ...
      if ( ( !isset($post->post_name) ) ||
           ( $post->post_name <= ' ' ) ||
           ( $post->ID != $product_id ) //v1.0.6 post info was getting lost when logging in on CHECKOUT
            ) {
         $post = get_post($product_id);
      //error_log( print_r(  '$post AFTER get_post= '  , true ) );
      ...
      //error_log( print_r(  '$post->ID != $product_id , $post->ID=  ' .$post->ID , true ) );      
         //save ID from the Post, which is the variation Parent
         $post_parent_ID = $post->ID;
         //get post for current Variation
         $post = get_post($product_id);
      //error_log( print_r(  '$post AFTER get_post PARENT= '  , true ) );
      //error_log( var_export($post, true ) );          
      } else {
        if ( $post->post_parent > 0 ) {
        
      //error_log( print_r(  '$post parent FOUND, use that instead!! $post= '  , true ) );
      //error_log( var_export($post, true ) );
      ...
      ************************************ */
      ...
      if ($post_parent_ID > 0) { //v1.0.6
        $use_this_id = $post_parent_ID;
//error_log( print_r(  'USE  $post_parent_ID ' .$post_parent_ID , true ) );         
      } else {
//error_log( print_r(  'USE   $product_id ' . $product_id , true ) );       
      ...
      //error_log( print_r(  '$use_this_id= ' .$use_this_id. ' prod_cat_list= '  , true ) );
      //error_log( var_export($vtwpr_cart_item->prod_cat_list, true ) );	  
 ...
 */
   ...
   //*************************
   // v1.0.66 new function
   //*************************
  function vtwpr_auto_update_setting_html1 ( $html, $plugin_file, $plugin_data ) {
    if ( 'wholesale-pricing-for-woocommerce/vt-wholesale-pricing.php' 	   ===  $plugin_file ) {
        $html = __( 'Auto-updates are not available for this plugin.', 'wholesale-pricing-for-woocommerce' );
    }
 
    return $html;
  }
  add_filter( 'plugin_auto_update_setting_html', 'vtwpr_auto_update_setting_html1', 10, 3 );

    
   //*************************
   // v1.0.66 new function
   //*************************
  function vtwpr_auto_update_setting_html2 ( $html, $plugin_file, $plugin_data ) {
    if ( 'wholesale-pricing-pro-for-woocommerce/vt-wholesale-pricing-pro.php' 	   ===  $plugin_file ) {
        $html = __( 'Auto-updates are not available for this plugin.', 'wholesale-pricing-pro-for-woocommerce' );
    }
 
    return $html;
  }
  add_filter( 'plugin_auto_update_setting_html', 'vtwpr_auto_update_setting_html2', 10, 3 );  